  ...
  <Teleport to="body">
    <transition name="rm">
      <div v-if="open" class="rm-scrim" @click.self="$emit('cancel')">
        <!-- Decorative orbital rings behind the modal -->
        ...
        >
          <!-- ═══ Ribbon header — leave type prominence ═══ -->
          <header class="rm-ribbon">
            ...
            >
              ...
              <LeaveTypeIcon v-if="leave?.leave_type" :type="leave.leave_type" :size="26" ambient />
            ...
            <div class="rb-meta">
              ...
              >
                ...
                <span class="rb-emp">{{ leave.employee_name || leave.employee_code || '—' }}</span>
                <span class="rb-sep">·</span>
                <span class="rb-days leave-mono">{{ leave.total_days }}d</span>
              </Motion>
            </div>

            <button class="rm-close" @click="$emit('cancel')" aria-label="Close">
              <X :size="14" />
            </button>
          </header>

          <!-- ═══ Body — preset reasons + custom note ═══ -->
          <div class="rm-body">
            ...
            >
              <header class="rm-sec-head">
                ...
                <span class="rm-sec-meta leave-mono">{{ presetReasons.length }} presets · {{ typeLabel.toLowerCase() }}</span>
              </header>

              <div class="rm-chips">
                <Motion v-for="(reason, i) in presetReasons" :key="reason" as="button" type="button"
                  class="rm-chip"
                  :class="{ 'is-active': selected.has(reason) }"
            ...
            >
              <header class="rm-sec-head">
                ...
                <span class="rm-sec-meta leave-mono" :data-warn="!finalReason.trim()">
                  {{ finalReason.length }}/1000
                </span>
              </header>

              <div class="rm-textarea-wrap" :class="{ 'is-focused': focused, 'is-empty': !finalReason.trim() }">
                <textarea ref="ta" v-model.trim="noteOnly" rows="3"
                  class="rm-textarea"
                ...
                <span class="rm-ta-corner tl" aria-hidden="true" />
                <span class="rm-ta-corner tr" aria-hidden="true" />
                <span class="rm-ta-corner bl" aria-hidden="true" />
                <span class="rm-ta-corner br" aria-hidden="true" />
              </div>
          ...
          >
            <button class="leave-btn leave-btn-sm rm-cancel" @click="$emit('cancel')">
              <ArrowLeft :size="13" /> Cancel
            </button>
            <Motion as="button" type="button"
              class="leave-btn leave-btn-sm leave-btn-danger rm-confirm"
...
import { XCircle, X, Check, ArrowLeft } from 'lucide-vue-next'
...
// Type-specific reason presets — copy is curated per leave type so the
// reviewer can pick a clearly-worded reason instead of typing from scratch.
const PRESETS_BY_TYPE = {
  CASUAL: [
    'Insufficient notice (< 24h)',
    'Conflicts with team sprint / event',
    'Coverage not arranged',
    'Casual balance exhausted',
    'Critical project deadline',
    'Frequent short-leave pattern',
  ],
  SICK: [
    'Medical certificate required (3+ days)',
    'Doctor\'s clearance pending',
    'Pattern of frequent sick leaves',
    'Documentation incomplete',
    'Half-day claim not eligible',
  ],
  EARNED: [
    'Notice period not met (< 15 days)',
    'Coverage not arranged',
    'Conflicts with critical deadline',
    'Pending approvals on related tasks',
    'Earned balance insufficient',
    'Suggest splitting into shorter spans',
  ],
  MATERNITY: [
    'Medical certificate pending',
    'Joining letter / start date confirmation needed',
    'HR clearance pending',
    'Documentation incomplete',
  ],
  PATERNITY: [
    'Birth proof / documentation pending',
    'Eligibility window expired (6 months)',
    'Notice period not met',
    'Manager endorsement missing',
  ],
  BEREAVEMENT: [
    'Relationship verification needed',
    'Documentation pending',
    'Outside policy scope (immediate family only)',
  ],
  COMP_OFF: [
    'Comp-off balance not validated',
    'Original OT day not approved',
    'Comp-off expiry exceeded (> 90 days)',
    'Insufficient comp-off credits',
  ],
  LWP: [
    'Insufficient justification',
    'Other paid balance still available',
    'Pattern of LWP usage',
    'Manager has not endorsed',
  ],
  STUDY: [
    'Course enrollment proof required',
    'Sponsorship terms unclear',
    'Manager endorsement pending',
    'Duration exceeds policy maximum',
  ],
  SPECIAL: [
    'Justification required',
    'Approval from senior management pending',
...
const typeLabel = computed(() => typeMeta(typeKey.value).label || 'this')
const presetReasons = computed(() => PRESETS_BY_TYPE[typeKey.value] || DEFAULT_PRESETS)
const placeholder = computed(() =>
  `Add more context for the employee${selected.value.size ? ' (optional)' : ' (required if no preset chosen)'}.`
)
...
const toggleReason = (r) => {
  ...
  // re-trigger reactivity since we mutate the Set in place
  selected.value = new Set(selected.value)
}

// Compose the final note that will be sent to the API.
...
watch(() => props.open, async (v) => {
  if (v) {
    ...
    focused.value = false
    await nextTick()
    // Don't autofocus textarea — let the user pick chips first
  }
})
</script>
...
/* ═══════════════════════════════════════════════════════════════════
   Scrim — radial ember backdrop with blur
   ═══════════════════════════════════════════════════════════════════ */
.rm-scrim {
...
.rm-ribbon {
  ...
  overflow: hidden;
  background:
    linear-gradient(90deg, rgba(234, 88, 12, 0.12), rgba(180, 83, 9, 0.04) 60%, transparent);
}
[data-theme="light"] .rm-ribbon {
  background: linear-gradient(90deg, rgba(234, 88, 12, 0.10), rgba(251, 191, 36, 0.08) 60%, transparent);
...
.rb-aura {
  ...
  width: 60%; height: 200%;
  background: radial-gradient(50% 30% at 50% 50%, rgba(234, 88, 12, 0.40), transparent 65%);
  filter: blur(28px);
  pointer-events: none;
}
...
.rb-glyph {
  ...
  border-radius: 16px;
  background:
    radial-gradient(60% 60% at 30% 30%, rgba(251, 191, 36, 0.30), transparent 70%),
    linear-gradient(135deg, rgba(234, 88, 12, 0.32), rgba(127, 29, 29, 0.28));
  border: 1px solid rgba(234, 88, 12, 0.42);
  color: #ffe0d0;
  box-shadow:
    0 12px 32px -10px rgba(234, 88, 12, 0.62),
...
[data-theme="light"] .rb-glyph {
  ...
  background:
    radial-gradient(60% 60% at 30% 30%, rgba(251, 191, 36, 0.30), transparent 70%),
    linear-gradient(135deg, rgba(251, 191, 36, 0.40), rgba(234, 88, 12, 0.30));
  border-color: rgba(194, 65, 12, 0.40);
}
.rb-glyph-aura {
  position: absolute; inset: -8px;
  border-radius: 50%;
  border: 1.5px solid rgba(234, 88, 12, 0.50);
  opacity: 0.7;
...
.rm-chip.is-active {
  background: linear-gradient(135deg, rgba(234, 88, 12, 0.30), rgba(180, 83, 9, 0.20));
  border-color: var(--leave-rejected);
  color: #fff7ed;
  font-weight: 700;
  box-shadow: 0 8px 20px -10px rgba(234, 88, 12, 0.50);
}
[data-theme="light"] .rm-chip.is-active {
  background: linear-gradient(135deg, rgba(234, 88, 12, 0.18), rgba(180, 83, 9, 0.10));
  color: #7c2d12;
...
.rm-ta-glow {
  ...
  background: linear-gradient(90deg, transparent, rgba(234, 88, 12, 0.10), transparent);
  background-size: 200% 100%;
  opacity: 0; pointer-events: none;
  transition: opacity .3s;
}
.rm-textarea-wrap.is-focused .rm-ta-glow {
  opacity: 1;
  animation: leave-gradient-pan 2.6s linear infinite;
}
.rm-ta-corner {
  position: absolute; width: 12px; height: 12px;
  border: 1.5px solid var(--leave-rejected);
  opacity: 0; pointer-events: none;
...
.rm-foot {
  ...
  background:
    linear-gradient(180deg, transparent, rgba(234, 88, 12, 0.04));
}
[data-theme="light"] .rm-foot {
  border-color: rgba(194, 65, 12, 0.16);
  background: linear-gradient(180deg, transparent, rgba(251, 191, 36, 0.06));
}
.rm-cancel:hover {
...
@media (prefers-reduced-motion: reduce) {
  .rm-orbit, .rb-glyph-aura, .rm-ta-glow { animation: none !important; }
}
